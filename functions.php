	...
	function lp_add_current_requests_query_args($permalink, $post)
	{
		...
		if (wp_doing_ajax()) {
			if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'ajax_search_tags') {
				if (isset($post) && $post->post_type == 'listing' && in_array($post->ID, $lp_ad_ids)) {
...
if (!function_exists('Listingpro_license_verification')) {
	...
	function replace_listing_author_image_with_business_logo($checked = false) {
		global $post, $listingpro_options;
	  
		// Check if business logo switch is enabled in theme options
		if( !empty($listingpro_options['business_logo_switch']) && $listingpro_options['business_logo_switch'] == true ) {
	  
		  // Get business logo for current listing
		  $business_logo = listing_get_metabox_by_ID('business_logo', get_the_ID());
	  
		  // Check if business logo is set for current listing
		  if( !empty($business_logo) ) {
			$business_logo_url = $business_logo;
		  } else {
			// If no business logo is set, use default logo specified in theme options
			$business_logo_url = $listingpro_options['business_logo_default']['url'];
		  }
	  
		  // Display business logo for current listing
		  echo '<div class="author-img-outer pos-relative custom-code-update">';
		  echo '<div class="author-img"><a href="' . get_the_permalink() . '"><img src="' . $business_logo_url . '" alt="Listing Logo"></a></div>';
		  if($checked) {
		  ...
		  // If business logo switch is not enabled, display author image as normal
		  echo '<div class="author-img-outer pos-relative custom-code-update">';
		  echo '<div class="author-img"><a href="' . get_author_posts_url(get_the_author_meta('ID')) . '"><img src="' . esc_url($avatar) . '" alt="image"></a></div>';
		  if($checked) {
			echo wp_kses_post($CHeckAd);
		  }
		  echo '</div>';
